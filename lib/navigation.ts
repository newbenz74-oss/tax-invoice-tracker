...
export interface NavLeaf {
  ...
  hidden?: boolean;
  /** เพิ่มเข้ามาพร้อมฟีเจอร์ "อนุมัติสมาชิกใหม่" รอบปรับปรุง (2026-08-10) — เมนูที่ตั้งค่านี้เป็น true จะ
   * แสดงเฉพาะกับบัญชีแอดมิน (ดู lib/adminAccess.ts) เท่านั้น สมาชิกคนอื่นทั้งหมดมองไม่เห็นเมนูนี้ใน Sidebar
   * เลย (ต่างจาก hidden ด้านบนที่ซ่อนจากทุกคนเหมือนกันหมด) — ค่าเริ่มต้นเป็น false/undefined (แสดงกับทุกคน)
   * ถ้าไม่ระบุ เช่นเดียวกับฟีเจอร์อื่นๆ ทั้งหมดในระบบที่ไม่มีแนวคิดสิทธิ์ระดับแอดมินเลย */
  adminOnly?: boolean;
}
...
export const NAV_STRUCTURE: NavEntry[] = [
  ...
  },
  {
    ...
    children: [
      ...
      // supabase/migration_026_audit_logs.sql และ components/ActivityLogPage.tsx
      //
      // วางไว้ใต้หมวด "ตั้งค่า" ไม่ใช่ระดับบนสุด เพราะเป็นเครื่องมือตรวจสอบย้อนหลัง ไม่ใช่งานประจำวันที่ต้อง
      // กดเข้าทุกวันแบบเมนูอื่น — ผู้ใช้จะเข้ามาก็ต่อเมื่อ "มีอะไรผิดปกติแล้วอยากรู้ว่าใครทำ" ซึ่งนานๆ ครั้ง
      //
      // ไม่ตั้ง adminOnly เพราะผู้ใช้เลือกไว้ว่าให้สมาชิกทุกคนในบริษัทเปิดดูได้ (2026-09-14) — RLS policy ใน
      // migration_026 ก็เปิดให้สมาชิกทุกคนอ่านสอดคล้องกัน ถ้าเปลี่ยนใจภายหลังต้องแก้ทั้งสองที่ให้ตรงกัน
      // (ตั้ง adminOnly: true ที่นี่อย่างเดียวแค่ซ่อนเมนู ไม่ได้ปิดการเข้าถึงข้อมูลจริง)
      { id: 'activity-log', label: 'ประวัติการใช้งาน', icon: ScrollText, implemented: true },
    ],
  },
  // เมนูใหม่ (2026-08-10) พร้อมระบบอนุมัติสมาชิกใหม่ — ผู้ใช้ระบุชัดเจนให้ย้ายมาไว้ล่างสุดของเมนูที่มองเห็นได้
  // (ต่อจากหมวด "ข้อมูลหลัก") และให้เห็นเฉพาะบัญชีแอดมินเท่านั้น (adminOnly: true — ดู lib/adminAccess.ts และ
