...
test.describe('เอฟเฟกต์เปลี่ยนหน้า Login → Dashboard', () => {
  test('กำลังเข้าสู่ระบบ: ปุ่มแสดง spinner + ข้อความ "กำลังเข้าสู่ระบบ..." และถูกปิดใช้งานจนกว่าจะเข้าหน้า Dashboard', async ({
    ...
    // (450ms) จะผ่านไป — enterDashboard() การันตีดีเลย์ขั้นต่ำนี้ก่อนนำทางจริงเสมอ (ดู app/login/page.tsx)
    // ตรวจทุกอย่าง (disabled/ข้อความ/spinner) พร้อมกันในการเรียก evaluate() เดียวจบ (ไม่แยกเป็นหลาย
    // expect()) เพื่อลดโอกาส "แข่งเวลา" กับ navigation ให้เหลือน้อยที่สุด — ยิ่งช่วงเวลาสั้นลง (450ms)
    // ยิ่งต้องระวังเรื่องนี้มากขึ้น ถ้าเครื่องช้าลงชั่วคราว (เช่น รันเทสต์คู่ขนานหลายตัวพร้อมกัน) แต่ละ
    // round-trip แยกกันมีโอกาสเจอช่วงที่ scheduler ดีเลย์สะสมข้ามเส้นได้ถ้าเรียกทีละครั้ง จึงรวมเป็นครั้ง
    // เดียวให้มากที่สุดเท่าที่ทำได้
