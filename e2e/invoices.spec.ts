...
test.describe('Invoice CRUD และ business logic', () => {
  ...
  // เพราะ StatsCards/MonthlyVatSummary ย้ายไปอยู่หน้า Dashboard ภาพรวมแทน (รอบปรับโครงสร้าง
  // Navigation/Layout 2026-07-15) เทสต์นี้เหลือแค่ตรวจคอลัมน์ยอดก่อน VAT/VAT ในตารางของหน้า
  // "บันทึกค่าใช้จ่าย" เอง
  test('แสดงยอดก่อน VAT / VAT ในตารางถูกต้อง', async ({ page }) => {
    const errors = attachConsoleErrorCollector(page);
    await setupMockSupabase(page, {
      loggedInAs: OWNER,
      users: [{ email: OWNER, password: 'x' }],
